...
DEFAULT_CONTROLLER_JUNIT = Path(r"C:\Codex\LitDataMatcher-v2\data\evaluation\E03_controller_independent.xml")
...
def selected_records(rows: list[dict[str, Any]], topic: str, count: int, excluded: set[str]) -> list[dict[str, Any]]:
...
def locator(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"url": row["source_locator"], "snapshot_sha256": snapshot["sha256"],
            "snapshot_url": snapshot.get("url"), "json_pointer": f"/result/{row['dataset_id']}"}
...
def question_for(row: dict[str, Any], split: str) -> dict[str, Any]:
    ...
    return {
        ...
        "split": split,
        "question": ("Find the source-described study matching this experimental context: " + row["title"] +
                     ". Required organism: " + row["organism"] + "; required assay: " + row["assay"] + "."),
        "requirements": [
            {"field": "organism", "expected": row["organism"], "essential": True, "source_locator": "frozen source-determined query"},
            {"field": "assay", "expected": row["assay"], "essential": True, "source_locator": "frozen source-determined query"},
            {"field": "study_title", "expected": row["title"], "essential": True, "source_locator": "frozen source-determined query"},
        ],
...
def ranking_metrics(order: list[str], labels: dict[str, int], k: int = 10) -> dict[str, Any]:
    ...
    return {
        ...
        "precision_at_5_numerator": hits if k == 5 else sum(gain > 0 for gain in gains[:5]), "precision_at_5_denominator": 5,
        "ndcg_at_5": (sum(gain / math.log2(index + 1) for index, gain in enumerate(gains[:5], 1)) /
                      (sum(gain / math.log2(index + 1) for index, gain in enumerate(sorted(labels.values(), reverse=True)[:5], 1)) or 1.0)),
...
def aggregate(metrics: list[dict[str, Any]]) -> dict[str, Any]:
    sums = Counter()
    for item in metrics:
        for key in ("queries", "candidate_relevance_labels", "positive_queries", "recall_at_10_numerator",
                    "recall_at_10_denominator", "precision_at_5_numerator", "precision_at_5_denominator", "invalid_top_match"):
            sums[key] += item[key]
    denominator = sums["recall_at_10_denominator"]
    return {**dict(sums), "recall_at_10": sums["recall_at_10_numerator"] / denominator if denominator else None,
            ...
            "invalid_top_match_rate": sums["invalid_top_match"] / sums["queries"] if sums["queries"] else None}
...
def capability_audit(profiles: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for profile in profiles:
        ...
        fields["reported_sample_count"] += int(profile["capabilities"]["reported_sample_count"]["status"] == "observed")
        fields["comparator_unknown"] += int(profile["capabilities"]["comparator"]["status"] == "unknown")
        if profile["independent_units"] is not None:
            failures.append(profile["dataset_id"] + ": inferred independent units")
    total = sum(fields.values())
    return {"field_family_counts": dict(fields), "correct_numerator": total - len(failures), "denominator": total,
            "correctness": (total - len(failures)) / total if total else None,
            "source_families": len(profiles), "failures": failures,
            "comparators_retained_unknown": fields["comparator_unknown"], "independent_donor_counts_inferred": 0}


def semantic_scores(model_dir: Path, texts: list[str], queries: list[str]) -> tuple[list[list[float]], dict[str, Any]]:
    from litdatamatcher.semantic_runtime import PretrainedSemanticIndex, verify_model
    manifest = verify_model(model_dir)
    index = PretrainedSemanticIndex(model_dir, device="cpu").fit([{"id": str(i), "text": value} for i, value in enumerate(texts)])
    ...
    for query in queries:
        ...
        matrix.append(row)
    return matrix, {"model_id": manifest["model_id"], "revision": manifest["revision"], "license": manifest["license"],
                    "runtime": "transformers PretrainedSemanticIndex", "device": "cpu", "cache_origin": "fresh_local_inference_no_cache"}
...
def scheduler_prerequisite_verdict(junit: Path) -> dict[str, Any]:
    ...
    return {"junit_path": str(junit), "tests": counts["tests"], "passed": passed, "failures": counts["failures"],
            "errors": counts["errors"], "skipped": counts["skipped"],
            "verdict": "TESTED_PREREQUISITE_READY_FOR_SCHEDULED_SUPERVISOR_REVIEW" if not any(counts[key] for key in ("failures", "errors")) else "PREREQUISITE_NOT_READY",
...
def evaluate_split(rows: list[dict[str, Any]], split: str, model_dir: Path) -> dict[str, Any]:
    ...
    by_method: dict[str, list[dict[str, Any]]] = {"lexical": [], "minilm_hybrid": [], "compatibility_aware": []}
    per_query: list[dict[str, Any]] = []
    for index, question in enumerate(questions):
        lexical = {row["dataset_id"]: lexical_score(question["question"], texts[pos]) for pos, row in enumerate(rows)}
    ...
    return {"split": split, "queries": questions, "candidate_profiles": profiles, "per_query": per_query,
            "metrics": {name: aggregate(values) for name, values in by_method.items()}, "capability_audit": capability_audit(profiles),
            "model": model}
...
def main() -> int:
    ...
    parser.add_argument("--labels", type=Path, default=Path(__file__).parent / "fixtures" / "evaluation_v2_labels.json")
    ...
    if [x["dataset_id"] for x in primary] != labels["primary_ids"] or [x["dataset_id"] for x in transfer] != labels["transfer_ids"]:
        raise ValueError("Acquired catalog no longer matches the frozen source-order selection fixture")
    started = time.perf_counter()
    result = {"protocol_id": labels["protocol_id"], "label_origin": labels["label_origin"], "expert_labels_available": False,
              "catalog": str(args.catalog), "catalog_sha256": hashlib.file_digest(args.catalog.open("rb"), "sha256").hexdigest(),
              "holdout_exposure": labels["holdout_exposure"], "reserved_excluded": sorted(excluded),
              ...
              "limitations": ["Metadata retrieval labels are source-determined, not expert biological fit labels.",
    ...
    result["gate_assessment"] = {"capability_floor": {"numerator": result["primary"]["capability_audit"]["correct_numerator"],
                                    "denominator": result["primary"]["capability_audit"]["denominator"], "threshold": 0.95,
                                    "pass": result["primary"]["capability_audit"]["correctness"] >= .95 and result["primary"]["capability_audit"]["source_families"] >= 10},
                               "primary_recall_floor": {"numerator": primary_metrics["compatibility_aware"]["recall_at_10_numerator"],
                                                        "denominator": primary_metrics["compatibility_aware"]["recall_at_10_denominator"], "threshold": .90,
                                                        "pass": primary_metrics["compatibility_aware"]["recall_at_10"] >= .90},
                               "invalid_top": {"count": primary_metrics["compatibility_aware"]["invalid_top_match"], "threshold": 0,
