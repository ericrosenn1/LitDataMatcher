#!/usr/bin/env python3
"""Streaming dataset-discovery worker.

The worker reads JSON lines from stdin and writes JSON lines to stdout so it can
be orchestrated as an isolated process. Each input row should contain a
``topic`` field. The output payload is generated by the reusable dataset
discovery node rather than a fixed mock.
"""

import os, sys, json, time, argparse, asyncio, threading
from collections import deque

from litdatamatcher.datasets import summarize_dataset_matches
...
async def fetch_summary(topic: str) -> dict:
    """Return a compact dataset-discovery summary for one topic."""

    return await asyncio.to_thread(summarize_dataset_matches, topic)
...
async def main():
    ...
    # Stdin loop
    # -----------------------------
    # A plain iterator is more portable than asyncio.connect_read_pipe on
    # Windows, and the actual network/catalog work remains async per batch.
