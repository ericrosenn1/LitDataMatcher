...
def integration_lease(root: str | Path, owner: str):
    ...
    try:
        ...
        locked = True
        atomic_json(root / 'owner.json', {'owner': owner, 'pid': os.getpid(),
                    'creation_time': psutil.Process().create_time(), 'acquired_at': time.time()})
        yield
    finally:
        if locked:
...
def is_alive(pid: int | None, creation_time: float | None) -> bool:
    ...
    try:
        ...
        return p.is_running() and abs(p.create_time() - creation_time) < .01
...
class Controller:
    ...
    def register(self, job_id: str, command: list[str], cwd: str | Path,
        ...
        outputs = [str((Path(p) if Path(p).is_absolute() else Path(cwd)/p).resolve()) for p in outputs]
        ...
        with self.db:
            ...
            self.db.execute('''INSERT OR IGNORE INTO jobs(id,command,cwd,outputs,status,fingerprint)
              VALUES(?,?,?,?,?,?)''', (job_id,json.dumps(command),cwd,json.dumps(outputs),'QUEUED',fp))
    ...
    def _check_outputs(self, job: dict, against_receipt=False) -> list[dict]:
        ...
        for value in json.loads(job['outputs']):
            ...
            data = p.read_bytes()
            if p.suffix == '.json':
                json.loads(data, parse_constant=lambda _: (_ for _ in ()).throw(ValueError('nonfinite JSON')))
            rows.append({'path':str(p),'sha256':hashlib.sha256(data).hexdigest(),'size_bytes':len(data)})
        if against_receipt:
    ...
    def repair(self) -> list[dict]:
        ...
        for job in self.jobs():
            if job['status'] == 'RUNNING':
                if is_alive(job['pid'],job['created']) or is_alive(job['owner_pid'],job['owner_created']):
                    continue
            ...
            quarantine = self.root/'quarantine'/f"{hashlib.sha256(job['id'].encode()).hexdigest()}-{time.time_ns()}"
            ...
            with self.db:
                self.db.execute("UPDATE jobs SET status='QUEUED',error=?,pid=NULL,created=NULL WHERE id=?",
                                (reason,job['id']))
    ...
    def run_next(self, timeout: float = 300, max_log_bytes: int = 2_000_000) -> dict:
        ...
        with integration_lease(self.root, 'deterministic-controller'):
            if self.mode() != 'RUNNING': return {'status': self.mode(), 'executed': False}
            repaired = self.repair()
            row = self.db.execute("SELECT * FROM jobs WHERE status='QUEUED' AND attempts<3 ORDER BY id LIMIT 1").fetchone()
            if row is None: return {'status':'IDLE','executed':False,'repairs':repaired}
            job = dict(row)
            # Prevent a stale result from being mistaken for success after a no-op command.
            before = {}
            for p in map(Path,json.loads(job['outputs'])):
                if p.is_file(): before[str(p)] = (p.stat().st_mtime_ns,hashlib.sha256(p.read_bytes()).hexdigest())
            ...
            try:
                ...
                process = subprocess.Popen(json.loads(job['command']),cwd=job['cwd'],stdout=subprocess.PIPE,
                                           stderr=subprocess.STDOUT,shell=False)
                created = psutil.Process(process.pid).create_time()
                with self.db:
                    self.db.execute('UPDATE jobs SET pid=?,created=? WHERE id=?',(process.pid,created,job['id']))
                import threading
                def drain():
                    retained = 0
                    with log.open('wb') as handle:
                        while chunk := process.stdout.read(8192):
                            ...
                            handle.write(chunk[:take]);retained += take
                ...
                while process.poll() is None:
                    if time.time()-started > timeout or self.mode() != 'RUNNING':
                        ...
                        break
                    time.sleep(.05)
                code = process.wait(timeout=10);reader.join(timeout=10)
                if reader.is_alive(): raise RuntimeError('Output drain incomplete')
                if failure or code != 0: raise RuntimeError(failure or f'Native exit {code}')
                ...
                atomic_json(self._receipt(job['id']), {'fingerprint':job['fingerprint'],'outputs':outputs,
                            'exit_code':code,'started':started,'finished':time.time()})
                status,error = 'SUCCEEDED',None
            except (OSError,ValueError,RuntimeError,subprocess.SubprocessError) as exc:
                code = process.returncode if process is not None else None
                status,error = ('QUEUED' if failure in {'PAUSED_BY_USER','WAITING_FOR_CAPACITY','STOPPED'} else 'FAILED'),str(exc)
            with self.db:
                if status == 'QUEUED':
                    self.db.execute('UPDATE jobs SET attempts=attempts-1 WHERE id=?',(job['id'],))
                self.db.execute('UPDATE jobs SET status=?,exit_code=?,error=?,finished=? WHERE id=?',
                                (status,code,error,time.time(),job['id']))
            return {'status':status,'job_id':job['id'],'exit_code':code,'error':error,'executed':True,'repairs':repaired}
...
def main(argv=None):
    ...
    args=p.parse_args(argv);c=Controller(args.root)
    try:
        if args.action == 'preflight': result={'mode':c.mode(),'jobs':c.jobs()}
        elif args.action == 'run-next': result=c.run_next()
        else:
            c.set_mode({'pause':'PAUSED_BY_USER','resume':'RUNNING','stop':'STOPPED'}[args.action])
            result={'mode':c.mode()}
