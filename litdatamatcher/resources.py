...
class ResourceGovernor:
    ...
    def admission(
        ...
        *,
        ...
        if (
            free < self.minimum_free_fraction
            or self.mode == "PRESSURE"
            and free < self.minimum_free_fraction + 0.05
        ):
            self.mode = "PRESSURE"
        ...
        workers = (
            ...
            else max(1, int(count * (0.7 if self.mode == "IDLE" else 0.35)))
        )
        return {
            "mode": self.mode,
            "admit_heavy": workers > 0,
            "cpu_workers": workers,
            "free_ram_fraction": free,
            "idle_sensor": "unavailable" if idle_seconds is None else "supplied",
