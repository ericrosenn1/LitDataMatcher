...
The frozen baseline and bounded ENA cache replay both passed after the policy ratchet. The next bounded V2.1 hardening task is a cross-source lifecycle merge: when a DOI duplicate carries a nested alternate-source correction, retraction, or version relation, it must remain ineligible for evidence and invalidate derived assertions. Add focused fixture tests and retain the existing public record shape; do not acquire live records, change alpha artifacts, or make a source-completeness claim.
