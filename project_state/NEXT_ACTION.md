...
The one-time v4 final holdout at `data\evaluation\final_holdout_v4\run` passed and is sealed: do not rerun it. Preserve completed acquisition, local model qualification, final integrated run, release archives, and deterministic test evidence. The corrective supervisor is paused and model/effort runtime-unverified, so it remains unavailable. The authoritative policy is `project_state/BUDGET_POLICY.md`: one `gpt-5.6-terra` worker at `low` effort, zero reasoning subagents, and a 22.90 percent duty cycle (at most 13.74 active minutes per hour followed by at least 46.26 minutes with no reasoning worker). The next telemetry sample is due after a further accumulated 30 minutes of Low work, a profile/concurrency/major-workload change, or `2026-09-08T11:50:57Z`, whichever occurs first.
...
The frozen baseline and bounded ENA cache replay both passed after the policy ratchet. The nested alternate-source correction/retraction/version lifecycle defect was corrected at lead commit `4cd9e9d83d210cdbdb2af1b30798b34bfc16539a`, with 309 passing tests. The next bounded V2.1 hardening task is exact cross-source identifier reconciliation without a DOI: current `_literature_identity` leaves `pubmed:12345` and `europepmc:MED:12345` separate even when both declare the exact same PMID. Give exact normalized PMID (and, where both sources declare it, exact PMCID) a documented identity key before source-specific fallback; preserve source-scoped provenance and do not merge on title similarity. Add focused fixtures for same-PMID merge and distinct/missing identifiers remaining separate. Do not acquire live records, change alpha artifacts, or make a source-completeness claim.

```powershell
C:\Codex\LitDataMatcher-v2\env\Scripts\python.exe -m pytest -q tests\test_adapters_and_capabilities.py tests\test_literature_integrity.py
```
